-- US-099 additive repair: terminal commands cannot predate their collection
-- chronology. The partial index supports the retained projection's sole audit scan.

CREATE INDEX idx_audit_collection_terminal_lookup
  ON audit_log_entry (org_id, subject_id, id)
  WHERE action_kind = 'collection.command.completed'
    AND subject_kind = 'extraordinary_collection';
