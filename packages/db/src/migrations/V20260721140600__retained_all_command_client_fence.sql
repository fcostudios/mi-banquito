-- US-099 additive repair: client-request uniqueness is evaluated across every
-- completed collection command before retained cancel/payout eligibility.
...
LANGUAGE sql STABLE SET search_path = pg_catalog, public AS $$
  WITH all_command_audits AS MATERIALIZED (
    SELECT a.org_id, a.subject_id AS collection_id, a.actor_id,
      a.actor_kind, a.reason, a.payload_snapshot AS payload,
      safe_collection_command_date(a.payload_snapshot) AS effective_on,
      count(*) OVER (
        PARTITION BY a.org_id, a.payload_snapshot->>'clientRequestId'
      ) AS client_request_event_count
    ...
    SELECT command.*,
      count(*) OVER (PARTITION BY command.org_id, command.collection_id) AS terminal_event_count
    FROM all_command_audits command
    WHERE command.payload->>'command' IN ('cancel', 'payout')
  ), authorized AS (
    ...
    WHERE event.terminal_event_count = 1
      AND event.client_request_event_count = 1
      AND event.effective_on IS NOT NULL
      AND event.effective_on >= header.opened_on
      AND NOT EXISTS (
        SELECT 1 FROM extraordinary_collection_line line
        WHERE line.org_id = header.org_id
          AND line.collection_id = header.id
          AND line.dated_on > event.effective_on
      )
