...
import { boolean, check, date, foreignKey, index, integer, jsonb, numeric, pgEnum, pgMaterializedView, pgTable, text, timestamp, unique, uniqueIndex, uuid } from "drizzle-orm/pg-core";
...
export const slip_photo_attached_to_kind_enum = pgEnum("slip_photo_attached_to_kind_enum", ["contribution", "repayment", "expense", "base_fund_quota_payment"]);
...
export const contribution = pgTable("contribution", {
  ...
  reconciliationStatus: reconciliation_status_enum("reconciliation_status").default("regularized").notNull(),
...
export const expense = pgTable("expense", {
  ...
  adjustmentCycleId: uuid("adjustment_cycle_id").references((): AnyPgColumn => reconciliationCycle.id),
  accountId: uuid("account_id"),
  category: expense_category_enum("category").notNull(),
  clientRequestId: uuid("client_request_id"),
  slipPhotoId: uuid("slip_photo_id"),
  createdAt: timestamp("created_at").notNull(),
  createdBy: uuid("created_by").notNull(),
  createdByKind: text("created_by_kind").notNull(),  // TODO[IMP-250]: enum members not cleanly parseable — text fallback
}, (table) => [
  ...
  foreignKey({
    ...
    columns: [table.slipPhotoId],
    foreignColumns: [slipPhoto.id],
  }),
]);
...
export const baseFundPoolPerFiscalYear = pgMaterializedView("mv_base_fund_pool_per_fiscal_year", {
  orgId: uuid("org_id").notNull(),
  fiscalYear: integer("fiscal_year").notNull(),
