/**
 * API client for the alerts bounded context.
...
export async function getAlerts(id: string) {
  return apiGet(`${PREFIX}/${id}`);
}

export async function createAlerts(data: unknown) {
  return apiPost(`${PREFIX}`, data);
}

export async function updateAlerts(id: string, data: unknown) {
  return apiPatch(`${PREFIX}/${id}`, data);
}

export async function deleteAlerts(id: string) {
