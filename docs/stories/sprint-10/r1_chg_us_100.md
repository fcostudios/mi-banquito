...
## User Story
As the rule-execution substrate (D-ARCH-1/2/3), I want a typed, zod-validated `config` lane plus a deterministic config resolver, so that config-driven rules (mora, two-pool) read the right values and the right version purely and replayably.
...
- [ ] AC-3: A rule **registry keyed by stable BR-id** (`packages/domain/rules/index.ts`) dispatches each rule; every output records which BR-id and which `group_config_version` produced it.
...
- **Business-rule execution:** `loadConfig(orgId, asOf) → frozen RuleContext`; registry dispatch by BR-id in `packages/domain/rules/index.ts`; temporal modes stamped / period-locked / per-accrual-day (SEC20). Reserved config keys: `config.mora`, `config.distribution`.
