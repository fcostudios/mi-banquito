...
## Acceptance Criteria
- [x] AC-1: A `/admin/orgs/[id]/reset-treasurer-login` action triggers an Auth0 **passwordless** `passwordless.start` with `connection: 'email'` for the org's treasurer.
- [x] AC-2: The action is **rate-limit aware**: at least **5 minutes** must elapse between attempts for the same treasurer; an earlier retry is rejected with a clear message and the remaining cooldown.
- [x] AC-3: Every re-issue attempt (success and rate-limited rejection) writes an `AuditLogEntry` with `actor_kind = 'platform_operator'`.
- [x] AC-4: The operator can notify the treasurer out-of-band via a **WhatsApp share** containing the operator's manual copy (share intent, not an automated message).
- [x] AC-5: The action targets only an org that has an existing treasurer `Member` (from US-018); orgs without one surface a clear precondition error.
