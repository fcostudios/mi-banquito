...
## Acceptance Criteria
- [x] AC-1: The member receives the statement PDF over WhatsApp as a result of the treasurer's share action (US-049) — no member-side app surface is built.
- [x] AC-2: The member can open the PDF preview in WhatsApp (the WhatsApp PDF preview is the surface; `n/a` screen in this app).
- [x] AC-3: No member login, account, or in-app screen is required or created in R1.
