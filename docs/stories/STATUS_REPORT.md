...
| US-003 | Partial / External Blocked | Local Docker Postgres flow works with schema apply/verify/seed. | Real Neon project, production branch, preview branch strategy, and target DB verification. |
...
| US-008 | Local Verified / External Blocked | Verifier asserts 34 tables, 29 RLS tables, 29 policy tables, 14 trigger tables, and 5 `updated_at` trigger tables. | Apply/verify against real Neon branch; deeper behavior for append-only/audit/period-lock remains later substrate stories. |
