...
- **AC (amended per CHG-011)**: an `Expense` (`category = treasurer_comp_payout`, label "pago a tesorera") is only accepted up to `payable_now` per amended BR-15, computed EXACTLY as: per-year `recognized_amount(Y) = max(BR-07 accrued compensation attributed to Y, Σ closed `ExtraordinaryCollection` `kind = treasurer_recognition` with `recognition_fiscal_year = Y`)` — max(), never sum (recognize once); `cumulative_entitlement(Y) = Σ over y ≤ Y of recognized_amount(y)`; `cumulative_paid(Y) = Σ(Withdrawal kind = treasurer_compensation_disbursement, period in any y ≤ Y — the US-050 cron cash-outs) + Σ(Expense category = treasurer_comp_payout in any y ≤ Y)`; `payable_now(Y) = max(0, cumulative_entitlement − cumulative_paid)` — the automated cron and this manual payout draw from ONE shared entitlement (DEC-MB-002, no double-payment); ceiling recomputed in the payout transaction (no TOCTOU); UI shows the full breakdown ("Reconocido $X · pagado $Y · disponible $Z" = cumulative entitlement/paid/available, pagado includes cron withdrawals) and **blocks** a payout above `payable_now` with typed error `compensation_ceiling_exceeded` carrying the three figures; the payout is deducted at share-out exactly as BR-11 states (a withdrawal, not part of the distributable pool); shows on public-verify + statements; audit log
- **Prerequisites**: US-091, US-017 (BR-07 config), US-050 (cron withdrawals the ceiling nets against — CHG-011), US-096 (recognition collection, optional path)
- **Info needed**: recognized_amount (accrual + collections), already_paid (cron withdrawals + prior payouts), payout amount
...
- **AC**: extend per-member statement (US-048), SCR-statements-archive, SCR-cash-flow-projection, SCR-year-end-share-out base, and SCR-public-verify-pdf to include `Expense` (by category), `Transfer` (regularizations visible), and `ExtraordinaryCollection` lines/payouts; fund balance shown is **net** of expenses; share-out base (BR-09) computes on the regularized group-fund balance; transparency test asserts every movement/transfer/collection in the period appears (no omissions)
